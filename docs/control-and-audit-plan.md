...
| Sync health | Encrypted repository reaches peer storage | Syncthing status, last sync time, repository size |
...
| Agent MVP | Telemetry allowlist, redacted logs, quota check, stale backup check, stale sync check |
...
| Encrypted repository synced to peer | Pass |
...
| Sync stale more than 72 hours | Critical | Alert both users, inspect Syncthing state |
